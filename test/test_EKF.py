...
def test_ekf_nees():
    ...
    tmt_times, tmt_states, tmt_measurement = combo.generate_truth_set(dt, 100, R_true, control[0:2], control[2:4])
    ...
    # Plot zeta_g
    axes[0].plot(tmt_times, ephemeris[:, 0], label='Estimated', color='blue')
    axes[0].fill_between(tmt_times, ephemeris[:, 0] - sigma_bounds[:, 0], 
                         ephemeris[:, 0] + sigma_bounds[:, 0], alpha=0.3, color='blue', label='2σ bounds')
    axes[0].plot(tmt_times, tmt_states[:, 0], color='r', linestyle='--', label='True')
    axes[0].set_ylabel('ζ_g (m)')
    axes[0].set_ylim([-100, 300])
    axes[0].set_title("Nees Testing")
    axes[0].legend()
    axes[0].grid(True)
    
    # Plot eta_g
    axes[1].plot(tmt_times, ephemeris[:, 1], label='Estimated', color='blue')
    axes[1].fill_between(tmt_times, ephemeris[:, 1] - sigma_bounds[:, 1], 
                         ephemeris[:, 1] + sigma_bounds[:, 1], alpha=0.3, color='blue', label='2σ bounds')
    axes[1].plot(tmt_times, tmt_states[:, 1], color='r', linestyle='--', label='True')
    axes[1].set_ylabel('η_g (m)')
    axes[1].set_ylim([-50, 50])
    axes[1].legend()
    axes[1].grid(True)
    ...
    # Plot theta_g
    axes[2].plot(tmt_times, ephemeris[:, 2], label='Estimated', color='blue')
    axes[2].fill_between(tmt_times, ephemeris[:, 2] - sigma_bounds[:, 2], 
                         ephemeris[:, 2] + sigma_bounds[:, 2], alpha=0.3, color='blue', label='2σ bounds')
    axes[2].plot(tmt_times, tmt_states[:, 2], color='r', linestyle='--', label='True')
    axes[2].set_ylabel('θ_g (rad)')
    axes[2].set_ylim([-4, 4])
    axes[2].legend()
    axes[2].grid(True)
    ...
    # Plot zeta_a
    axes[3].plot(tmt_times, ephemeris[:, 3], label='Estimated', color='blue')
    axes[3].fill_between(tmt_times, ephemeris[:, 3] - sigma_bounds[:, 3], 
                         ephemeris[:, 3] + sigma_bounds[:, 3], alpha=0.3, color='blue', label='2σ bounds')
    axes[3].plot(tmt_times, tmt_states[:, 3], color='r', linestyle='--', label='True')
    axes[3].set_ylabel('ζ_a (m)')
    axes[3].set_ylim([-300, 100])
    axes[3].legend()
    axes[3].grid(True)
    ...
    # Plot eta_a
    axes[4].plot(tmt_times, ephemeris[:, 4], label='Estimated', color='blue')
    axes[4].fill_between(tmt_times, ephemeris[:, 4] - sigma_bounds[:, 4], 
                         ephemeris[:, 4] + sigma_bounds[:, 4], alpha=0.3, color='blue', label='2σ bounds')
    axes[4].plot(tmt_times, tmt_states[:, 4], color='r', linestyle='--', label='True')
    axes[4].set_ylabel('η_a (m)')
    axes[4].set_ylim([-100, 500])
    axes[4].legend()
    axes[4].grid(True)
    ...
    # Plot theta_a
    axes[5].plot(tmt_times, ephemeris[:, 5], label='Estimated', color='blue')
    axes[5].fill_between(tmt_times, ephemeris[:, 5] - sigma_bounds[:, 5], 
                         ephemeris[:, 5] + sigma_bounds[:, 5], alpha=0.3, color='blue', label='2σ bounds')
    axes[5].plot(tmt_times, tmt_states[:, 5], color='r', linestyle='--', label='True')
    axes[5].set_xlabel('Time (s)')
    axes[5].set_ylabel('θ_a (rad)')
    axes[5].set_ylim([-4, 4])
    axes[5].legend()
